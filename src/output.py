...
def write_parameters(parameter_dict, array):
    print("if no input, project will be named myProject by default")
    ...
    with open(project_name, "w", newline="") as csvfile:
        ...
        # writer.writerow([project_name[:-4], current_time, parameter_dict['z'], parameter_dict['w'], parameter_dict['l'],
    ...
    writer.writerow([project_name[:-4], current_time])
    writer.writerow([""])
    ...
    writer.writerow([parameter_dict["Ph"], round(parameter_dict["gammaP"], 2)])
    writer.writerow([""])
    ...
    writer.writerow([parameter_dict["elevation"], round(parameter_dict["Ke"], 2)])
    writer.writerow([""])
    ...
    writer.writerow(
        [
            array.panel_list[0].length,
            array.panel_list[0].width,
            array.panel_list[0].length * array.panel_list[0].width,
        ]
    )
    ...
    writer.writerow([""])
    return project_name, file, writer


def write_panels(project_name, file, writer, parameter_dict, array):
    panel_parameters = {}
    ...
    for panel in array.panel_list:
        ...
        panel_parameters["panel coords"] = list(panel.polygon.exterior.coords[:-1])
        panel_parameters["panel class"] = panel.panel_class
        writer.writerow(
            [
                panel.index,
                list(panel.polygon.exterior.coords[:-1]),
                panel.panel_class,
                panel.zones,
                panel.vortex_zones,
                panel.gamma_e,
                round(
                    parameter_dict["qz"] * parameter_dict["gammaP"] * panel.gamma_e, 1
                ),
                panel.load_share_factor,
                round(panel.AtribL, 2),
                round(panel.AnL, 2),
                round(panel.gcl, 2),
                round(panel.forceL),
                round(panel.AtribS, 2),
                round(panel.AnS),
                round(panel.gcs, 2),
                round(panel.forceS),
                round(min(panel.forceS, panel.forceL)),
            ]
        )
